...
int main(int argc, char **argv)
{
    ...
    if (argc > 3) {
        ...
        if (argv[3][1] == '*') items_count = strtoul(argv[3] + 2, 0, 0);
    }
    ...
    /* Map one page */
    // printf("mmap(%d, %d, 0x%x, 0x%x, %d, 0x%x)\n", 0, map_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, (int)
    // target);
    ...
    for (i = 0; i < items_count; i++) {
        ...
        if (verbose)
            printf("Value at offset 0x%X (%p): 0x%0*lX\n", (int)target + i * type_width, virt_addr, type_width * 2,
                   read_result);
        else {
